...
def calculate_spectrum_features(time_series, fs):
    ...
    if not idx_total.size: 
        return {
            ...
            'snr_db': -100.0, 'hr_peak_purity': 0.0
        }
...
def extract_segment_features(segment_data: Dict[str, Any], fs: float, window_duration_s: float) -> Dict[str, Any]:
    ...
    for r in range(R):
        for v in range(nVX):
            ...
            segment_features['amp_hr_peak_purity'][r, v] = amp_spec_feats['hr_peak_purity']
...
# 🔥 核心修改：支持动态窗口和步长
# ========================================================================
def process_single_file(file_info: Dict[str, Any], window_duration_s: float, 
                       step_duration_s: float, keep_raw_phase: bool = False) -> List[Dict[str, Any]]:
    """ 
    ...
    参数:
        ...
        step_duration_s: 步长（秒）
        keep_raw_phase: 是否保留原始phase数据（测试时需要）
    """
    ...
    try:
        with h5py.File(file_path, 'r') as f:
            ...
            step_size_frames = int(step_duration_s * target_fs)
            
            if int(target_fs) != 20:
                print(f"⚠️ 警告: 文件 {os.path.basename(file_path)} 的采样率是 {target_fs:.1f} Hz。")
            ...
            R = filtered_1_2hz_phase_fft.shape[0]
            T = filtered_1_2hz_phase_fft.shape[2]
            ...
            # 🔥 滑动窗口切片（支持重叠）
            num_segments = (T - window_size_frames) // step_size_frames + 1
            
            print(f"✅ 文件: {os.path.basename(file_path)} | 距离: {target_distance}cm | T={T} | "
                  f"窗口: {window_duration_s}s ({window_size_frames}帧) | 步长: {step_duration_s}s ({step_size_frames}帧) | "
                  f"生成 {num_segments} 个片段...")
            ...
            for i in range(num_segments):
                ...
                # 🔥 如果需要保留原始phase数据（测试时）
                if keep_raw_phase:
                    segment_data['raw_phase_data'] = filtered_1_2hz_phase_fft[:, :, start_frame:end_frame].copy()
...
# ========================================================================
# 🔥 主函数：批量处理所有文件（支持动态窗口和步长）
# ========================================================================
def extract_features_from_all_files(file_configs: List[Dict], 
                                   window_duration_s: float = 30,
                                   step_duration_s: float = 30,
                                   keep_raw_phase: bool = False) -> List[Dict[str, Any]]:
    """
    批量处理所有实验文件，提取特征
    ...
    print(f"--- 开始批量处理 {len(file_configs)} 组实验数据 ---")
    print(f"   窗口长度: {window_duration_s}s | 步长: {step_duration_s}s | 保留原始phase: {keep_raw_phase}")
    
    for info in file_configs:
        result = process_single_file(info, window_duration_s, step_duration_s, keep_raw_phase)
        if result:
    ...
    print(f"总共生成的特征片段数: {total_segments}")
    print("=" * 50)
...
if __name__ == "__main__":
    ...
    # 测试训练配置（30s无重叠）
    print("\n=== 测试训练配置 ===")
    train_results = extract_features_from_all_files(
        TEST_FILE_CONFIGS, 
        window_duration_s=30, 
        step_duration_s=30,
        keep_raw_phase=False
    )
    print(f"训练模式生成片段数: {len(train_results)}")
    
    # 测试测试配置（120s, 15s步长）
    print("\n=== 测试测试配置 ===")
    test_results = extract_features_from_all_files(
        TEST_FILE_CONFIGS, 
        window_duration_s=120, 
        step_duration_s=15,
        keep_raw_phase=True
    )
    print(f"测试模式生成片段数: {len(test_results)}")
    if len(test_results) > 0:
        print(f"首个片段是否包含raw_phase_data: {'raw_phase_data' in test_results[0]}")
        if 'raw_phase_data' in test_results[0]:
            print(f"raw_phase_data形状: {test_results[0]['raw_phase_data'].shape}")